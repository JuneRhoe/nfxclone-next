...
export const drizzleMgTableIdSeqInDevelop = develop.sequence(
  'drizzle-mg-table_id_seq',
  {
    startWith: '1',
    increment: '1',
    minValue: '1',
...
export const nfxcloneSessionsInDevelop = develop.table(
  'nfxclone_sessions',
  {
  ...
  (table) => [unique('nfxclone_sessions_user_id_key').on(table.userId)],
)
...
export const mediaMainCategoriesInDevelop = develop.table(
  'media_main_categories',
  {
    id: smallint().primaryKey().generatedByDefaultAsIdentity({
      name: 'develop.media_main_category_id_seq',
      startWith: 1,
      increment: 1,
      minValue: 1,
      maxValue: 32767,
...
export const nfxcloneUsersInDevelop = develop.table(
  'nfxclone_users',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [unique('nfxclone_users_user_id_key').on(table.userId)],
)

export const mediasInDevelop = develop.table(
  'medias',
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const browsePreviewMediaListInDevelop = develop.table(
  'browse_preview_media_list',
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: 'number' }).primaryKey().generatedByDefaultAsIdentity({
      name: 'develop.media_preview_id_seq',
      startWith: 1,
      increment: 1,
      minValue: 1,
...
export const mediaBrowseDisplayInDevelop = develop.table(
  'media_browse_display',
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: 'number' }).primaryKey().generatedByDefaultAsIdentity({
      name: 'develop.media_browse_display_id_seq',
      startWith: 1,
      increment: 1,
      minValue: 1,
      maxValue: 9223372036854775807,
